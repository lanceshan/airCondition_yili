...
{					//(0,1,2,0)
    ...
    if(AChn >= MSCAN_CHANNELS_MAX) return ERR_CHN; 						 /* check the channel used or not */
    AChn_Transformed = vChnIdx[AChn];	//0
    if (AChn_Transformed == 0xff) return ERR_CHN;
...
*
**************************************************************************/
uint8 Load_CAN_MB ( uint8 AChn, uint8 ABuffer, uint8 Datalen, uint8 *ADataPtr)
{
    uint8 *bufferPtr;             
    uint8 i;
    uint8 AChn_Transformed;
    ...
    (p_Buffer+ABuffer)->DLC = Datalen;                                 /* copy DLC */

    bufferPtr = (p_Buffer+ABuffer)->Data;                                                                     /* increment to start of data */
...
uint8 Read_CAN_MB_Data(uint8 AChn, uint8 ABuffer, uint8* ADataPtr)
{
    ...
    datalen = Read_MB_Data (AChn, ABuffer, ADataPtr);
